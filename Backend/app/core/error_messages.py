...
GENERIC_ERROR_MESSAGE = "操作失败，请稍后重试。"
...
ERROR_MESSAGES_ZH: dict[str, str] = {
    ...
    "audio_duration_too_long": "单次录音时长超过当前识别上限，请控制在 60 秒以内。",
...
def normalize_error_detail(detail: Any) -> tuple[str, str]:
    if isinstance(detail, str):
        code = detail.strip() or "request_failed"
        return code, ERROR_MESSAGES_ZH.get(code, GENERIC_ERROR_MESSAGE)
    if isinstance(detail, list):
        return "request_validation_failed", ERROR_MESSAGES_ZH["request_validation_failed"]
    return "request_failed", GENERIC_ERROR_MESSAGE
