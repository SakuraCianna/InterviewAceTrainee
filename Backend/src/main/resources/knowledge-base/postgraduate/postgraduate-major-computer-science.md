---
id: postgraduate-major-computer-science
domain: postgraduate
category: major
title: 计算机科学与技术复试专业能力练习
aliases: 计算机科学与技术,computer-science
tags: 复试,专业能力,数据结构、操作系统、网络、数据库、算法与工程实践
...
## 适用边界
本材料用于 计算机科学与技术 的代表性专业复试训练，不替代目标院校当年专业目录、参考书目和复试细则。不同院系即使专业名称相同，研究方向和评价方式也可能不同；教育部规定之外的具体要求必须以招生单位官网为准。

## 核心知识与表达
- **知识主线**：围绕数据结构、操作系统、网络、数据库、算法与工程实践建立概念网络。不能只背定义，要能解释概念之间的因果、层级或约束关系，并指出常见混淆。
...
## 研究与实践问题
专业锚点问题：为高并发数据服务选择索引、缓存和一致性策略，并用复杂度与故障实验验证。
