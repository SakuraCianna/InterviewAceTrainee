...
- 场景预设题库：不同训练模块使用独立题池，考公按结构化题型抽题，雅思按 Part 1/2/3 抽取连贯话题，研究生复试会根据用户填写的院校信息做启发式难度分层。
...
后端会优先从 `Backend/app/services/interview_question_bank.py` 生成每场训练的问题序列；如果题库生成失败，才回退到旧的基础问题。题库规则：

- 工作面试：结合简历、岗位名称、JD 和岗位预设方向抽取问题。
- 研究生复试：结合目标院校、报考专业和研究方向抽题；院校层级只是训练强度启发式，不宣称官方排名。
